...
def _deserialize_to_object(data: Any) -> Any:
    ...
    if isinstance(data, dict) and "__class__" in data:
        ...
        if class_name == "ProxyListResponse":
            from ..schemas.proxy import ProxyListResponse
            processed_data = _deserialize_to_object(data)
            return ProxyListResponse.parse_obj(processed_data)
        elif class_name == "ProxyResponse":
            from ..schemas.proxy import ProxyResponse
            processed_data = _deserialize_to_object(data)
            return ProxyResponse.parse_obj(processed_data)
    ...
    if isinstance(data, dict) and "__class__" in data:
        ...
        if class_name == "Proxy":
            ...
            proxy = Proxy()
            for key, value in data.items():
                if key != "__class__" and hasattr(proxy, key):
                    # 递归处理嵌套对象
                    deserialized_value = _deserialize_to_object(value)
                    setattr(proxy, key, deserialized_value)
            return proxy
        elif class_name == "ProxyQuality":
            from ..models.proxy import ProxyQuality
            # 创建ProxyQuality对象
            quality = ProxyQuality()
            for key, value in data.items():
                if key != "__class__" and hasattr(quality, key):
                    # 递归处理嵌套对象
                    deserialized_value = _deserialize_to_object(value)
                    setattr(quality, key, deserialized_value)
            return quality
